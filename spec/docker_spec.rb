...
RSpec.describe 'docker build', if: ENV['DOCKER_BUILD_SERVER'] do
  before(:all) do
    ...
    @eslint = { linter: "#{@dckr}/eslint.yml", result: "#{@dckr}/eslint_result.txt" }.merge(@js_repo)
    @jshint = { linter: "#{@dckr}/jshint.yml", result: "#{@dckr}/jshint_result.txt" }.merge(@js_repo)
    @nsp = { linter: "#{@dckr}/nsp.yml", result: "#{@dckr}/nsp_result.txt" }.merge(@js_repo)
    @ocstyle = { linter: "#{@dckr}/ocstyle.yml", result: "#{@dckr}/ocstyle_result.txt" }.merge(@ios_repo)
    @sasslint = { linter: "#{@dckr}/sasslint.yml", result: "#{@dckr}/sasslint_result.txt" }.merge(@scss_repo)
    @scsslint = { linter: "#{@dckr}/scsslint.yml", result: "#{@dckr}/scsslint_result.txt" }.merge(@scss_repo)
    @stylint = { linter: "#{@dckr}/stylint.yml", result: "#{@dckr}/stylint_result.txt" }.merge(@stylus_repo)
    @tailor = { linter: "#{@dckr}/tailor.yml", result: "#{@dckr}/tailor_result.txt" }.merge(@swift_repo)
    @gometalinter = { linter: "#{@dckr}/gometalinter.yml", result: "#{@dckr}/gometalinter_result.txt" }.merge(@go_repo)
    @credo = { linter: "#{@dckr}/credo.yml", result: "#{@dckr}/credo_result.txt" }.merge(@elixir_repo)
    @scala = { linter: "#{@dckr}/scalastyle.yml", result: "#{@dckr}/scalastyle.txt" }.merge(@scala_repo)
